...
SAVE_PATH = Path(os.path.expanduser('~/.ros/wf_qtable.npy'))
LOG_PATH  = Path(os.path.expanduser('~/.ros/wf_train_log.csv'))

class QLearningTrainer(Node):
    def __init__(self):
        super().__init__('q_learning_trainer')
        # --- config ---
        self.algorithm = 'q_learning'
        self.reward_mode = 'sparse'      # 'sparse' or 'shaped'
        self.goal = (2.6, 3.1, 0.5)      # x, y, radius
        ...
        # --- keepalive so Gazebo never sleeps ---
        self.last_cmd = Twist()
        ...
        # 5 scenarios to satisfy Deliverable 2
        self.scenarios = [
        ...
        self.start_episode()
        self.get_logger().info("Q-Learning trainer ready (sparse reward, episodic)")
    ...
    def try_load_qtable(self):
        if SAVE_PATH.exists():
            try:
                ...
                if isinstance(data,dict): self.q_table.update(data); self.get_logger().info(f"Loaded Q table from {SAVE_PATH}")
    ...
    def teleport(self,x,y,yaw):
        if not self.entity_name:
            self.get_logger().warn("Teleport skipped (no entity)."); return
        try:
            req=SetEntityPose.Request()
    ...
    def get_sector_avg(self,ranges,a,b):
        n=len(ranges)
        vals = (list(ranges[int(a*n/360):])+list(ranges[:int(b*n/360)])) if a>b else list(ranges[int(a*n/360):int(b*n/360)])
    ...
    def determine_state(self,scan):
        ...
        s=(self.dist_to_state(L,'left'), self.dist_to_state(F,'front'), self.dist_to_state(RF,'right_front'), self.dist_to_state(R,'right'))
        return s,(L,F,RF,R)
    ...
    def eps_greedy(self,s):
        if np.random.rand()<self.epsilon: return np.random.randint(len(self.action_names))
    ...
    def start_episode(self):
        ...
        self.get_logger().info(f"[TRAIN][Q] Episode {self.ep} start | ε={self.epsilon:.3f}")

    def end_episode(self,reason='timeout'):
        self.get_logger().info(f"[TRAIN][Q] Ep{self.ep} end | steps={self.step} return={self.ep_return:.1f} | {reason}")
        try:
            ...
            with open(LOG_PATH,'a',newline='') as f:
                ...
                w.writerow([self.ep,round(self.ep_return,2),self.step,reason,'q_learning'])
        except Exception: pass
    ...
    def scan_cb(self,msg: LaserScan):
        ...
        if self.step%500==0:
            np.save(SAVE_PATH,self.q_table)
            self.get_logger().info(f"[TRAIN][Q] autosave step {self.step} ret={self.ep_return:.1f}")

def main():
    rclpy.init()
    node = QLearningTrainer()
    try:
        rclpy.spin(node)
